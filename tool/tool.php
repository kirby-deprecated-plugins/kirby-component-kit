<?php
require_once __DIR__ . DS . 'lib' . DS . 'finder.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes.php';
require_once __DIR__ . DS . 'lib' . DS . 'render.php';

require_once __DIR__ . DS . 'lib' . DS . 'api' . DS . 'globals.php';
#require_once __DIR__ . DS . 'lib' . DS . 'api' . DS . 'core.php';
require_once __DIR__ . DS . 'lib' . DS . 'api' . DS . 'components.php';
require_once __DIR__ . DS . 'lib' . DS . 'api' . DS . 'file.php';
require_once __DIR__ . DS . 'lib' . DS . 'api' . DS . 'files.php';
require_once __DIR__ . DS . 'lib' . DS . 'api' . DS . 'image.php';
require_once __DIR__ . DS . 'lib' . DS . 'api' . DS . 'bar.php';
require_once __DIR__ . DS . 'lib' . DS . 'api' . DS . 'raw.php';

require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'default.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'render-image.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'render-raw.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'tool-code.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'tool-image.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'tool-html.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'tool-home.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'tool-preview.php';
require_once __DIR__ . DS . 'lib' . DS . 'routes' . DS . 'tool-missing.php';